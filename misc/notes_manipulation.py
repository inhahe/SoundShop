import re, pprint, types, fractions
...
#todo: we can remove some of these note changing functions since we're going to do a midi graph instead
...
#ScheduledNotesNode:
#DelaySamples
#HumanizeSamples (jitter start/duration by ±N samples, seeded RNG); Random or algorithmic microtiming (milliseconds) is naturally absolute-time.
#SwingSamples / GrooveSamples (if you treat groove as sample offsets)
#Stutter/RatchetSamples (repeat at given Hz or sample interval)
#LimitDensity (drop/merge events when too dense)
#ClampToGridSamples (optional utility)

#Free-time / rubato passages; Where notes follow expressive timing curves not tied cleanly to beats.
#Granular / glitch / stutter / ratchet; “Repeat this slice at 200 Hz for 0.3 seconds” is an absolute-time spec.
#Live-coding / event scheduling; “Play this note 120 ms after that one” is absolute.
#Tempo derived from audio-rate modulation; If you ever let MIDI react to LFOs/envelopes/audio analysis, you’ll often get time-based events.
#Precise sync to wall-clock / SMPTE / video; Absolute time is the native coordinate.

class NotesNode:
  def __init__(self, function=None): # ex: `function=lambda n: Note(n, velocity=max(n.velocity+20, 127))` or `function=velocity(lambda n: n+20)`
    ...
    #TransposeBeat
    #ScaleConstrainBeat
    #QuantizeBeat(to rational grids: 1 / 16, triplets, etc.)
    #HumanizeBeat(optional: only if you keep it beat - relative)
    #GateBeat(change durations)
    #SplitBeat(partition notes into branches; enforce non - overlap if you want)

# see soundshop_misc\comping 2.txt
class NotesFilter:
  def __init__(self, function=None): # ex: `function=lambda n: 1000 < n.sampleOffset < 2000`
    self.function = function
    # filters select which notes to change with notesnodes before they get to the instrument.
    # you can have multiple filters on the same note arrangement with separate notesgraphs
    # but if two filters overlap at any note, then they must use the same notesgraph or an exception will be raised.
...
class NotesGraph:
  ...
  def addfilter(self, filter):
    if type(filter) is types.FunctionType:
      self.filters.append(NotesFilter(filter))
    elif type(filter) is NotesFilter:
      self.filters.append(filter)
    else:
      raise ValueError

  def addconnection(self, connection1, connection2): # connection1 can be a NotesNode or a NotesFilter.
...
class Note: #todo: make properties to change things. for example, if you change degree or key, then note, letter, accidental, pitch class and midi change too. we could cheat and use Note(self, degree=new_degree)
  ...
  def __init__(self, note=None, accidental=None, octave=None, key=None, mode=0, degree=None, velocity=127, beatDuration=1, beatNumber=None, beatInterval=None, playOrder=None):
    assert not (beatInterval and beatNumber)
    self.beatInterval = beatInterval # beatInterval is the number of beats since the previous note in the Notes object
    ...
    if type(note) is Note:
      ...
      self.sampleDuration = note.sampleDuration
      self.samplenumber = note.sampleNumber
      self.beatNumber = note.beat
      self.beatDuration = note.beatDuration
      self.time = note.time
      self.timeDuration = note.timeDuration
    ...
    if sampleDuration is not None:
      self.sampleDuration = sampleDuration
    if beatNumber is not None:
      self.beatNumber = beatNumber
    if beatDuration is not None:
      self.beatDuration = beatDuration
    if timeOffset is not None:
      self.timeOffset = timeOffset
    if timeDuration is not None:
      self.timeDuration = timeDuration
    assert sum(x is not None for x in (self.sampleNumber, self.beatNumber, self.timeOffset)) <= 1
    assert sum(x is not None for x in (self.sampleDuration, self.beatDuration, self.timeDuration)) <= 1
...
class Notes(list): #todo: __repr__ and __str__
  def __init__(self, notes=None, key=None, bpm=None, mode=0, accidental=None, sampleOffset=None, beatNumber=None, timeOffset=None, sampleDuration=None, beatDuration=None, timeDuration=None, orderOffset = 0):
    assert (beatNumber is not None) + (timeOffset is not None) + (sampleOffset is not None) <= 1 and (sampleDuration is not None) + (beatDuration is not None) + (timeDuration is not None) <= 1
