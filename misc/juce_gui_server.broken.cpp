...
private:
    // Add this member variable to store found plugins
    ...
    Response scanForPlugins(const std::string& directory)
    {
        ...
        juce::MessageManager::callAsync([this, directory, &resp]() {
            juce::File pluginDir;
...
Response listAvailablePlugins()
    {
        ...
        // Format: "index:name:path,index:name:path,..."
    ...
    void scanDefaultPluginDirectories()
    {
        ...
        // Common Windows VST directories
    ...
    void scanPluginDirectory(const juce::File& directory)
    {
        ...
        {
            juce::FileSearchPath searchPath(directory.getFullPathName());
            
            for (int i = 0; i < searchPath.getNumPaths(); ++i)
            {
                juce::File path = searchPath[i];
                
                // Find all plugin files
                juce::Array<juce::File> pluginFiles;
                
                if (auto* vstFormat = dynamic_cast<juce::VSTPluginFormat*>(format))
                {
                    // Scan for VST plugins
                    pluginFiles.addArray(path.findChildFiles(juce::File::findFiles, false, "*.dll"));
                }
                else if (auto* vst3Format = dynamic_cast<juce::VST3PluginFormat*>(format))
                {
                    // Scan for VST3 plugins
                    pluginFiles.addArray(path.findChildFiles(juce::File::findDirectories, false, "*.vst3"));
                }
                ...
                for (const auto& pluginFile : pluginFiles)
                {
                    ...
                    {
                        // Create a copy and add to our list
