...
public:
  ...
  void timerCallback() override 
  {
    ...
    for (auto it = pluginWindows.begin(); it != pluginWindows.end();) 
    {
      ...
      cout << "pluginWindow " << it->first << " pointer: " << it->second << endl;
  ...
  */
  ...
  private:
    ...
    void renderOffline()
    {
      ...
      uint64_t totalBlocks = playbackEndBlock;
  ...
  void startAudioProcessing(uint64_t endBlock, const std::string& outputFile = "")
  {
    playbackEndBlock = endBlock;
    if (renderToFile && !outputFile.empty())
    {
      // Setup file writer for offline rendering
  ...
  void stopAudioProcessing()
  {
    if (renderToFile)
    {
      audioFileWriter.reset();  // Close file
    }
    else
    {
...
#define READFROMPIPE(type) readFromPipe<type>()
  ...
  getParamsInfoR getParamsInfo(int id) 
  {
    getParamsInfoR resp;
    vector<paramInfo> validParams;
...
/*
  ...
  void processCommand(char command)
  {
    ...
    switch (commandtype)
    {
      ...
      case start_playback:
      {
        string fileName = READFROMPIPE(string);
        uint64_t endBlock = READFROMPIPE(uint64_t);
        startAudioProcessing(endBlock, fileName);
        break;
      }
      case stop_playback:
      {
      ...
      {
        uint32_t a = READFROMPIPE(uint32_t); //debug
        getParamsInfoR resp = getParamsInfo(a); 

        cout << "lpindex: " << a << endl; 

        cout << "success: " << resp.success << " originalNumParams: " << resp.originalNumParams << " errmsg: " << resp.errmsg << endl;

        WRITEALL(resp.success, resp.validParams.size(), resp.errmsg);
