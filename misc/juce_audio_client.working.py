...
class JuceAudioClient:
    ...
    def _connect_windows(self) -> bool:
        ...
        try:
            # Use simple file I/O since that's what worked in our tests
    ...
    def _send_command(self, command_type: str, params: Dict[str, str] = None) -> Tuple[bool, str, Dict[str, str]]:
        ...
        try:
            ...
            # Use simple file I/O for both Windows and Unix since it worked in our tests
            ...
            def read_response():
                ...
                try:
                    data = self.pipe_handle.read(4096)
                    if data:
            ...
            if not response_received:
                # No response is often normal for some commands
                response = "OK:Command sent successfully"
