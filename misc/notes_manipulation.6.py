...
# Generate scales for all 12 keys in all 7 modes
scales = []
# Major scale intervals (Ionian mode)
major_intervals = [2, 2, 1, 2, 2, 2, 1]

for root in range(12):  # 12 possible root notes
  for mode in range(7):  # 7 modes
    cur_semi = root
    scale_s = set()
    scale_l = []
    # Rotate intervals based on mode
    mode_intervals = major_intervals[mode:] + major_intervals[:mode]
    for i in mode_intervals:
      scale_s.add(cur_semi)
      scale_l.append(cur_semi)
      cur_semi = (cur_semi + i) % 12
    # Only add unique scales
    if (scale_s, scale_l) not in scales:
      scales.append((scale_s, scale_l))

print(f"Number of unique scales: {len(scales)}")  # Should be 12 (one for each key)
...
print(f"{changed_semitones=}")

#should get F# G# A B C# D E
#gets 

print(f"{get_keys(get_semitones('C D E F G A B'))=}")
print(f"{changed_semitones=}")
keys = get_keys(changed_semitones)
print(f"{keys=}")
key = keys[0]
print(f"{key=}")
key = key[0]
a = semitones_dict[key]
print(f"{a=}")
print(get_notes(changed_semitones, "D"))
