...
def _node_key(node: Signal, child_ids: Tuple[int, ...]) -> Tuple[Any, ...]:
    ...
    if isinstance(node, WavetableOsc):
        # frequency child + table + params
        return (t, tuple(node.table), child_ids[0], node.phase0, node.interp, node.loop, node._cache_every_blocks)
...
class SignalEngine:
    ...
    def compile(self) -> CompiledSignalGraph:
        ...
        for k, sig in list(self._roots.items()):
            self._resolve_refs(sig)

        # Canonicalize roots (dedupe)
        for k, sig in list(self._roots.items()):
            res = canonicalize(sig)
            self._roots[k] = res.root
