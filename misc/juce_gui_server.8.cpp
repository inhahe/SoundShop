...
private:
  ...
  void processScheduledChanges() 
  {
    while (scheduler.lastChangeIndex < scheduler.scheduledChanges.size() && 
      scheduler.currentBlock == scheduler.scheduledChanges[scheduler.lastChangeIndex].atBlock)
    {
      ScheduledParameterChange& change = scheduler.scheduledChanges[scheduler.lastChangeIndex];
      setPluginParameter(change.lpindex, change.parameterIndex, change.value);
      change.executed = true;
      scheduler.lastChangeIndex++;
    }
  }
