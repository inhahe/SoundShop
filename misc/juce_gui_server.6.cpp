...
#define READFROMPIPE(type) readFromPipe<type>()
  ...
  struct paramInfo
  {
    ...
    float value;
    uint32_t numSteps = -1;
    uint32_t isDiscrete = -1;
    uint32_t isBoolean = -1;
    uint32_t isOrientationInverted = -1;
    uint32_t isAutomatable = -1;
    uint32_t isMetaParameter = -1;
  };
  ...
  void processCommand(char command)
  {
    ...
    switch (commandtype)
    {
      ...
      case get_params_info:
      {
        ...
        {
          WRITEALL(param.name, param.minValue, param.maxValue, param.interval, param.skewFactor, param.value, param.numSteps, param.isDiscrete, param.isBoolean, 
