...
class send_cmd:
  ...
  connect_audio, connect_midi, start_playback, cmd_shutdown, remove_plugin, list_bad_paths, get_params_info, get_channels_info, \
  schedule_midi_note, schedule_midi_cc, clear_midi_schedule = range(20)

class recv_cmd:
  param_change, param_changes_end, stop_playback = range(3)
...
class JuceAudioClient:
    ...
    def startplayback(self, endblock, tofile, filename):
      ...
      self.sendinfo("II", endblock, tofile)
...
def main():
  ...
  while True:
    cmd = client.readinfo("B")
    if cmd==recv_cmd.param_change:
      lpindex, parameterIndex, value = client.readinfo("III")
      print(f"parmeter change: {lpindex=}, {parameterIndex=}, {value=}")
