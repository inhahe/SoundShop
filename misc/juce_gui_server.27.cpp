...
struct ScheduledParameterChange
{
    ...
    int parameterIndex;
    float value;
    int64_t atSample;   // absolute timeline sample
};
...
#ifdef _WIN32
    if (GetLastError() == ERROR_BROKEN_PIPE) { notificationPipeReady = false; break; }
#else
    if (errno == EPIPE) { notificationPipeReady = false; break; }
#endif
  }
}
...
#define READFROMPIPE(type) readFromPipe<type>()
  ...
  void renderToFile(uint64_t endBlock, string outputFile)
  {
    ...
    for (uint64_t block = 0; block < totalBlocks; ++block)
    {
        ...
        int64_t blockStart = paramScheduler.getCurrentSample();
  ...
  const juce::AudioIODeviceCallbackContext& context)
{
    ...
    if (host)
    {
        ...
        ::currentSamplePosition = host->paramScheduler.getTimelineSample();
