...
def loadplugin(self, path: str, key: int):
    ...
    # C++: success(u32), name(str), uid(u32), errmsg(str)
    success = int(self.readinfoc("I")[0])
    name = self.readstr()
    uid = int(self.readinfoc("I")[0])
    errmsg = self.readstr()
...
def getParamsInfo(self, pluginKey: int) -> list[dict]:
    ...
    return params
def getChannelsInfo(self, pluginKey: int) -> dict:
    ...
    for _ in range(int(num_input_buses)):
        ...
        channelTypes = [self.readstr() for _ in range(int(numChannelTypes))]
        (isEnabled,) = self.readinfoc("I")
        mainBusLayout = self.readstr()
        in_buses.append({
    ...
    def schedulemidicc(self, *args): #todo: add to song
      ...
      self.sendinfo("IIId", *args)
      self.commands_pipe_handle_flush()
    
def schedulemidiccs(self, ccs):
    """ccs: iterable of (pluginKey, controller, value_0_127, time_seconds, channel)"""
...
def scheduleparamchanges(self, changes, *, epsilon: float = 0.0):
    """changes: iterable of (pluginKey,paramIndex,value_float,atSample)."""
...
def connectaudio(self, sourceId: int, sourceChannel: int, destId: int, destChannel: int) -> int:
    ...
    self.commands_pipe_handle_flush()
    return int(self.readinfoc("I")[0])
def startplayback(self, endSample: int, *, toFile: bool = False, filename: str = "") -> int:
    """Start playback/render until timeline reaches endSample (absolute sample count)."""
...
def scheduleaudioregion(self, playerId: int, startSample: int, fileStartSample: int) -> int:
    """Schedule audio playback (server action=2)."""
...
def commands_pipe_handle_flush(self) -> None:
    """Flush the command pipe so the server receives bytes promptly."""
    try:
...
def stopplayback(self) -> int:
    self.sendcmd(send_cmd.stop_playback)
    self.commands_pipe_handle_flush()
    return int(self.readinfoc("I")[0])


def loadaudiofileintoplayer(self, filename: str) -> int:
    """Load an audio file into a new server-side AudioFilePlayerNode. Returns playerId or -1."""
...
# This is the “DAW style” smoothing you typically see on modulation sources to avoid zipper noise.
    ...
    def _eval(self, sample: int, ctx: EvalContext, cache: Optional[ControlCache]) -> float:
        ...
        return yk
"""
Why Smooth has its own cache (not ControlCache)
ControlCache caches a signal’s value per block, but smoothing needs the previous output to compute the next output. That’s a recurrence, so it needs a per-node history cache.
...
class Track: # input node = "input", output node = "output", that way we can put tracks into a graph
  ...
  def __init__(self, id, name, parent_id=None, children=None, graph=None, audioTracks=None, midiSchedules=None, notes=None, notesGraph=None, processorGraph=None, midiCcSchedules=None,
    ...
    self.graph = graph or SimplePortGraph()
