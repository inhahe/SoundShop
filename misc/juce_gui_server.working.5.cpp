...
public:
          ...
          running(true)
    {
        ...
        std::cout << "Added AU format" << std::endl;
#endif
...
private:
                       ...
                       )
    {
        ...
        else if (cmd.type == "load_plugin_by_index")
        {
            response = loadPluginByIndex(cmd.params["index"], cmd.params["id"]);
        }
        else if (cmd.type == "scan_plugins")
        {
            response = scanForPlugins(cmd.params["directory"]);
        }
        else if (cmd.type == "list_plugins")
        {
            response = listAvailablePlugins();
        }
        else if (cmd.type == "get_plugin_info")
        {
            response = getPluginInfo(cmd.params["id"]);
        }
    ...
    void scanDefaultPluginDirectories()
    {
        ...
        for (const auto& dirPath : defaultDirs)
        {
    ...
    {
        std::cout << "Scanning directory: " << directory.getFullPathName() << std::endl;
        ...
        std::cout << "Found " << allFiles.size() << " files in directory" << std::endl;
        ...
        for (const auto& file : allFiles)
        {
            ...
            if (extension == ".dll" || extension == ".vst" || extension == ".vst3")
            {
                ...
                std::cout << "  -> Added as potential plugin" << std::endl;
            }
        }
    ...
    Response scanForPlugins(const std::string& directory)
    {
        Response resp;
    ...
    Response listAvailablePlugins()
    {
        ...
        std::cout << "List plugins requested. Current plugin count: " << availablePlugins.size() << std::endl;
        
        if (availablePlugins.size() == 0)
        {
            std::cout << "No plugins in list, auto-scanning..." << std::endl;
            scanDefaultPluginDirectories();
            std::cout << "After auto-scan, plugin count: " << availablePlugins.size() << std::endl;
        }
        
        resp.success = true;
        resp.message = "Plugin list";
        
        if (availablePlugins.size() == 0)
        {
            resp.data["plugin_list"] = "";
            resp.data["count"] = "0";
            std::cout << "Sending empty plugin list" << std::endl;
        }
        else
        {
            ...
            for (int i = 0; i < availablePlugins.size(); ++i)
            {
                ...
                // Use safer string concatenation and escape any problematic characters
                std::string name = desc->name.toStdString();
                std::string path = desc->fileOrIdentifier.toStdString();
                
                // Replace any commas or colons in names/paths to avoid parsing issues
                std::replace(name.begin(), name.end(), ':', '_');
                std::replace(name.begin(), name.end(), ',', '_');
                std::replace(path.begin(), path.end(), ',', '|');  // Use | instead of comma for paths
                
                pluginList += std::to_string(i) + ":" + name + ":" + path;
                
                std::cout << "Added to list: " << i << " - " << name << std::endl;
            }
            
            resp.data["plugin_list"] = pluginList;
            resp.data["count"] = std::to_string(availablePlugins.size());
            
            std::cout << "Sending plugin list with " << availablePlugins.size() << " plugins" << std::endl;
            std::cout << "List data: " << pluginList.substr(0, 200) << "..." << std::endl;
        }
