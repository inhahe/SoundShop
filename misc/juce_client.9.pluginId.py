...
#the index isit's what's returned as 'pluginId' (loaded plugin index) when you load a plugin.
#similarly for parameters, to control them use their 'originalIndex'
#update: now it's pluginId, the unique id associated with the plugin. 
#todo: but does this work for AU files? what do we do about that? and what does pluginId return for AU files?
...
class Processor:
  def __init__(self, pluginInfo=None, connectedTo=None): #connectedTo should be a list
