...
def add_to_song(song_name, item):
  if type(item) is ParametersSchedule:
...
def del_from_song(song_name, item):
  if type(item) is ParametersSchedule:
  elif type(item) is MidiSchedule:
  elif type(item) is Notes:
  elif type(item) is AudioTrack:
...
class send_cmd:
  ...
  schedule_ordered_notes, start_ordered_playback, stop_ordered_playback, clear_ordered_notes = range(37)
...
class JuceAudioClient:
    ...
    def getPluginInfo(self, lpindex):
    ...
    while True:
      try:
        ...
        if cmd==recv_cmd.param_change:
          ...
          lpindex, samplePosition = client.readinfon("iQ")
          if lpindex == -1:
            print(f"MIDI keyboard unrouted at sample {samplePosition}")
          else:
            print(f"MIDI keyboard routed to plugin {lpindex} at sample {samplePosition}")
        elif cmd==recv_cmd.virtual_keyboard_routed:
          lpindex, samplePosition = client.readinfon("iQ")
          if lpindex == -1:
            print(f"Virtual keyboard unrouted at sample {samplePosition}")
          else:
            print(f"Virtual keyboard routed to plugin {lpindex} at sample {samplePosition}")
