...
public:
  void processBlockWithPluginIds(std::unordered_map<int, juce::MidiBuffer>& buffersByPlugin, 
    int blockSize) 
  {
    // Clear all buffers
    ...
    while (nextEventIndex < scheduledEvents.size()) //todo: figure this out and see if it's as efficient as my solution in the param scheduler
    {
      ...
      if (event.samplePosition >= currentSamplePosition) 
      {
        ...
        buffersByPlugin[event.pluginId].addEvent(event.message, sampleOffset);
      }
      nextEventIndex++;
    }
...
public:
  MidiSourceNode(MidiScheduler* s, int pluginId)
  ...
  {
    midiMessages.clear();
    scheduler->getEventsForPlugin(targetPluginId, midiMessages, buffer.getNumSamples());
  }

  // Required AudioProcessor methods
  const juce::String getName() const override { return "MIDI Source " + juce::String(targetPluginId); }
...
public:
  ...
  unordered_map<int, juce::AudioProcessorGraph::NodeID> pluginIds; 
...
#define WRITEALLN(...) writeAlln(__VA_ARGS__)
  ...
  void sendQueuedParameterNotifications() {
    ...
    while (!pendingNotifications.empty())
    {
      ...
      cout << "sending parameter notification" << " pluginId: " << event.pluginId << " parameterIndex: " << event.parameterIndex << " value: " << event.value << " atBlock: " << event.atBlock << endl;
      WRITEALLN(param_changed, event.pluginId, event.parameterIndex, event.value, event.atBlock);
#ifdef _WIN32
  ...
  void setupAudioIO()
  {
    ...
    pluginIds[outputIndex] = audioOutputNode;  // Special ID for audio output
    pluginIds[inputIndex] = audioInputNode;   // Special ID for audio input
  }
  ...
  */
  ...
  void toggleMidiKeyboardRouting(int pluginId)
  {
    cout << "toggleMidiKeyboardRouting called with lpindex=" << lpindex << ", current keyboardRoutedPlugin=" << keyboardRoutedPlugin << endl;
    if (keyboardRoutedPlugin == pluginId)
    {
      // Unroute if already routed to this plugin
      keyboardRoutedPlugin = -3;
      cout << "MIDI keyboard unrouted from plugin " << pluginId << endl;
    }
    else
    {
      // Route to this plugin
      keyboardRoutedPlugin = pluginId;
      cout << "MIDI keyboard routed to plugin " << pluginId << endl;
    }
  ...
  void toggleVirtualKeyboardRouting(int pluginId)
  {
    cout << "toggleVirtualKeyboardRouting called with lpindex=" << pluginId << ", current virtualKeyboardRoutedPlugin=" << virtualKeyboardRoutedPlugin << endl;
    if (virtualKeyboardRoutedPlugin == pluginId)
    {
      // Unroute if already routed to this plugin
      virtualKeyboardRoutedPlugin = -1;
      cout << "Virtual keyboard unrouted from plugin " << pluginId << endl;
    ...
    {
      // Route to this plugin
      virtualKeyboardRoutedPlugin = pluginId;
      cout << "Virtual keyboard routed to plugin " << pluginId << endl;
    ...
    cout << "About to send WRITEALLN notification: virtual_keyboard_routed, pluginId=" << virtualKeyboardRoutedPlugin << ", samplePos=" << currentSamplePosition << endl;
    // Send notification to Python client
  ...
  void updateAllRoutingIndicators()
  {
    ...
    juce::MessageManager::callAsync([this]() {
      ...
      for (auto& pair : pluginWindows)
      {
        cout << "  Updating window for pluginId=" << pair.first << ", window ptr=" << (void*)pair.second.get() << endl;
        if (pair.second)
        {
    ...
    const juce::MidiMessage& message) override
  {
    ...
    if (message.isController())
    {
      ...
      for (const auto& mapping : ccMappings)
      {
        ...
        {
          auto it = pluginIds.find(mapping.lpindex);
          if (it != pluginIds.end())
          {
            auto node = processorGraph->getNodeForId(it->second);
            if (node && node->getProcessor())
...
#define READFROMPIPE(type) readFromPipe<type>()
  ...
  unordered_map<int, juce::AudioProcessorGraph::NodeID> midiSourceNodes;  // pluginId -> MIDI source node
  ...
  getParamsInfoR getParamsInfo(int pluginId) 
  {
    getParamsInfoR resp;
    vector<paramInfo> validParams;

    auto it = pluginIds.find(lpindex);
    if (it != pluginIds.end()) {
      auto node = processorGraph->getNodeForId(it->second);
  ...
  getChannnelsInfoR getChannelsInfo(int pluginId) 
  {
    getChannnelsInfoR resp;
   
    cout << "Looking for plugin with pluginId: " << pluginId << endl;
  ...
  void getPluginInfo()
  {
    int pluginId = READFROMPIPE(uint32_t);
    auto plugin = availablePlugins[pluginId];
    WRITEALLC(uint32_t(plugin.desc.isInstrument), uint32_t(plugin.desc.uniqueId), uint32_t(plugin.desc.numInputChannels), uint32_t(plugin.desc.numOutputChannels), plugin.desc.name.toStdString(),
        plugin.desc.descriptiveName.toStdString(), plugin.desc.pluginFormatName.toStdString(), plugin.desc.category.toStdString(), plugin.desc.manufacturerName.toStdString(), plugin.desc.version.toStdString(),
        plugin.desc.fileOrIdentifier.toStdString(), plugin.desc.lastFileModTime.toString(true, true).toStdString(), plugin.path);
  }
  ...
  struct loadPluginByIndexR {uint32_t success = true; string name; int32_t pluginId = -3; /*-1 and -2 are taken*/ uint32_t uid = -1; string errmsg;};
    ...
    if (apindex >= 0 && apindex < availablePlugins.size()) 
    {
      ...
      if (plugin != nullptr) 
      {
        ...
        if (node != nullptr) 
        {
          ...
          pluginIds[pluginInfo.desc.uniqueId] = node->nodeID;
          //processorTolpindex[processorPtr] = lpindex;
          resp.pluginId = pluginInfo.desc.uniqueId;
          ...
          if (midiScheduler)  // Make sure scheduler exists
          {
            auto midiSource = std::make_unique<MidiSourceNode>(
              midiScheduler.get(), pluginInfo.desc.uniqueId);
            auto sourceNode = processorGraph->addNode(std::move(midiSource));

            if (sourceNode)
            {
              // Store the source node
              midiSourceNodes[pluginInfo.desc.uniqueId] = sourceNode->nodeID;
  ...
  struct loadPluginR {uint32_t success = false; string errmsg; string name; int32_t pluginId = -3; uint32_t uid;}; //todo: change uint to int for pluginId in pipe communication
  ...
  showPluginUIR showPluginUI(int pluginId)
  {
    showPluginUIR resp;
    auto it = pluginIds.find(pluginId);
    if (it != pluginIds.end())
    {
      auto node = processorGraph->getNodeForId(it->second);
      if (node)
      {
        juce::MessageManager::callAsync([this, pluginId, node]() {
          cout << "showPluginUI async callback executing for pluginId=" << pluginId << endl;
          if (node->getProcessor()->hasEditor())
          {
            ...
            if (editor)
            {
              ...
              auto window = make_unique<PluginWindow>(node->getProcessor()->getName(), editor, node->getProcessor(), pluginId, this);
              ...
              pluginWindows[pluginId] = std::move(window);
              cout << "  showPluginUI async callback completed for pluginId=" << pluginId << endl;
            }
            else
            {
              cout << "  ERROR: Failed to create editor for pluginId=" << pluginId << endl;
            }
          }
          else
          {
            cout << "  Processor does not have editor for pluginId=" << pluginId << endl;
          }
...
*/
    ...
    {
      resp.success = false;
      resp.errmsg = "Plugin not loaded: " + to_string(pluginId);
    }
    return resp;
  }

  uint32_t hidePluginUI(int pluginId)
  {
    auto it = pluginWindows.find(pluginId);
  ...
  void removePlugin(int pluginId) 
  {
    ...
    auto sourceIt = midiSourceNodes.find(pluginId);
  ...
  void processCommand(char command)
  {
    ...
    switch (commandtype)
    {
      case load_plugin:
      {
        ...
        WRITEALLC(response.success, response.name, response.pluginId, response.uid, response.errmsg);
        break;
      }
      case remove_plugin:
      {
      ...
      case load_plugin_by_index:
      {
        ...
        cout << "load_plugin_by_index:" << endl << "index: " << index << " success: " << response.success << " name: " << response.name 
          << " pluginId: " << response.pluginId << " uid: " << response.uid << " errmsg: " << response.errmsg << endl;
      ...
      {
        uint32_t pluginId = READFROMPIPE(uint32_t); //debug
        getParamsInfoR resp = getParamsInfo(pluginId); 
      ...
      case schedule_midi_note:
      {
        ...
        midiScheduler->scheduleNote(pluginId, note, velocity, startTime, duration, channel);
        break;
      }
      case schedule_midi_cc:
      {
        ...
        int channel = READFROMPIPE(uint32_t);
        midiScheduler->scheduleCC(pluginId, controller, value, time, channel);
        break;
      }
      ...
      case schedule_param_change:
      {
        ...
        scheduler.scheduleParameterChange(pluginId, parameterIndex, value, atBlock);
        break;
      }
      case route_keyboard_input:
      {
        ...
        cout << "Keyboard routed to plugin " << pluginId
      ...
      case route_cc_to_param:
      {
        ...
        cout << "CC " << ccController << " (channel " << midiChannel
             << ") routed to plugin " << pluginId << " param " << parameterIndex << endl;
      ...
      case unroute_cc_to_param:
      {
        ...
        ccMappings.erase(
          std::remove_if(ccMappings.begin(), ccMappings.end(),
            [pluginId, parameterIndex, ccController](const CCMapping& m) {
        ...
        cout << "Unrouted CC " << ccController << " from plugin " << pluginId
      ...
      case route_virtual_keyboard:
      {
        ...
        cout << "Virtual keyboard routed to plugin " << pluginId
  ...
  public:
    ...
    RoutingButton(RoutingType type, int pluginUniqueId, CompletePluginHost* hostApp)
      : routingType(type), pluginId(pluginUniqueId), host(hostApp), isActive(false)
    {
      cout << "RoutingButton constructor: type=" << type << ", pluginId=" << pluginUniqueId << endl;
      setSize(40, 40);
    ...
    {
      cout << "RoutingButton::paint called for pluginId=" << pluginId << ", type=" << routingType << ", isActive=" << isActive << endl;
      ...
      cout << "RoutingButton::paint completed for pluginId=" << pluginId << endl;
    }

    void mouseDown(const juce::MouseEvent& event) override
    {
      cout << "RoutingButton::mouseDown for pluginId=" << pluginId << ", type=" << routingType << endl;
